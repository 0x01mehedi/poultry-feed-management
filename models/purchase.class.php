...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		//$result=$db->query("select id,supplier_id,purchase_date,delivery_date,shipping_address,purchase_total,paid_amount,remark,status_id,discount,vat from {$tx}purchases $criteria limit $top,$perpage");
		//select p.id,s.name supplier,p.purchase_date,p.delivery_date,p.shipping_address,p.purchase_total,p.paid_amount,p.remark,ss.name status,p.discount,p.vat from core_purchases p,core_suppliers s,core_status ss where p.supplier_id=s.id and p.status_id=ss.id
		...
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-warning\" href=\"create-purchase\">New Purchase</a></th></tr>";
		if($action){
			$html.="<tr><th>Id</th><th>Suppliers</th><th>Purchase Date</th><th>Delivery Date</th><th>Shipping Address</th><th>Purchase Total</th><th>Paid Amount</th><th>Remark</th><th>Status</th><th>Discount</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Supplier Id</th><th>Purchase Date</th><th>Delivery Date</th><th>Shipping Address</th><th>Purchase Total</th><th>Paid Amount</th><th>Remark</th><th>Status Id</th><th>Discount</th></tr>";
		}
		while($purchase=$result->fetch_object()){
			$action_buttons = "";
			if($action){
				...
				$action_buttons.= action_button(["id"=>$purchase->id, "name"=>"Edit", "value"=>"Edit", "class"=>"secondary", "url"=>"edit-purchase"]);
				$action_buttons.= action_button(["id"=>$purchase->id, "name"=>"Delete", "value"=>"Delete", "class"=>"danger", "url"=>"purchases"]);
				$action_buttons.= "</div></td>";
			}
			$html.="<tr><td>$purchase->id</td><td>$purchase->supplier</td><td>$purchase->purchase_date</td><td>$purchase->delivery_date</td><td>$purchase->shipping_address</td><td>$purchase->purchase_total</td><td>$purchase->paid_amount</td><td>$purchase->remark</td><td>$purchase->status</td><td>$purchase->discount</td> $action_buttons</tr>";
		}
		$html.="</table>";
