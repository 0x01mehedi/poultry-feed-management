...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		//$result=$db->query("select id,name,contact_category_id,contact_no,email from {$tx}contacts $criteria limit $top,$perpage");
		$result=$db->query("select c.id,cc.name category,c.name,c.contact_no,c.email from {$tx}contacts c,{$tx}contact_categories cc where c.contact_category_id=cc.id $criteria limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-warning\" href=\"create-contact\">New Contact</a></th></tr>";
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Contact Categorys</th><th>Contact No</th><th>Email</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Contact Categorys</th><th>Contact No</th><th>Email</th></tr>";
		}
		while($contact=$result->fetch_object()){
			$action_buttons = "";
			if($action){
				...
				$action_buttons.= action_button(["id"=>$contact->id, "name"=>"Edit", "value"=>"Edit", "class"=>"secondary", "url"=>"edit-contact"]);
