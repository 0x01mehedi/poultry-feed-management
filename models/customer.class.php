...
";
	...
	static function html_select($name="cmbCustomer"){
		global $db,$tx;
		$html="<select id='$name' name='$name' class='p-0 m-0'> ";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-warning\" href=\"create-customer\">New Customer</a></th></tr>";
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Mobile</th><th>Email</th><th>Created At</th><th>Address</th><th>Photo</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Mobile</th><th>Email</th><th>Created At</th><th>Address</th><th>Photo</th></tr>";
		}
		while($customer=$result->fetch_object()){
			$action_buttons = "";
			if($action){
				...
				$action_buttons.= action_button(["id"=>$customer->id, "name"=>"Edit", "value"=>"Edit", "class"=>"secondary", "url"=>"edit-customer"]);
				$action_buttons.= action_button(["id"=>$customer->id, "name"=>"Delete", "value"=>"Delete", "class"=>"danger", "url"=>"customers"]);
				$action_buttons.= "</div></td>";
			}
			$html.="<tr><td>$customer->id</td><td>$customer->name</td><td>$customer->mobile</td><td>$customer->email</td><td>$customer->created_at</td><td>$customer->address</td><td><img src='img/$customer->photo' width='100' /></td> $action_buttons</tr>";
		}
		$html.="</table>";
